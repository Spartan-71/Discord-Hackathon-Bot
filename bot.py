...
class HackathonPaginator(discord.ui.View):
    ...
    @discord.ui.button(label="◀️ Previous", style=discord.ButtonStyle.gray, custom_id="prev_hack", row=1)
    async def previous_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.current_index > 0:
            ...
            await interaction.response.send_message("⚠️ You're already at the first hackathon!", ephemeral=True)
    ...
    async def next_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.current_index < self.max_index:
            ...
            await interaction.response.send_message("⚠️ You're already at the last hackathon!", ephemeral=True)
    
    @discord.ui.button(label="🗑️ Close", style=discord.ButtonStyle.red, custom_id="close_hack", row=1)
    ...
    async def _update_message(self, interaction: discord.Interaction):
        ...
        try:
            await interaction.response.edit_message(
                content=msg if msg else f"📖 Hackathon {self.current_index + 1}/{len(self.hackathons)}:",
                embed=embed,
                view=self
            )
        except discord.NotFound:
            await interaction.response.send_message("⚠️ Message was deleted. Please run the command again.", ephemeral=True)
...
async def send_scheduled_notification_with_pagination(channel, hackathons):
    ...
    await channel.send(
        content=f"📖 **Detailed View** (1/{len(hackathons)}):",
        embed=embed,
        view=paginator
    )
...
async def send_hackathon_notifications(bot, new_hackathons, target_channel=None):
    ...
    if target_channel:
        ...
        try:
            ...
            logging.info(f"Sent {len(new_hackathons)} hackathons to channel {target_channel.id} with pagination")
        except discord.Forbidden:
            logging.error(f"403 Forbidden when sending to channel {target_channel.id}. Check permissions.")
        except Exception as e:
            logging.error(f"Failed to send hackathon notifications to channel {target_channel.id}: {e}")
    else:
        db = SessionLocal()
        try:
            for guild in bot.guilds:
                ...
                if filtered_hackathons:
                    try:
                        ...
                        logging.info(f"Sent {len(filtered_hackathons)} hackathons to guild {guild.id} with pagination")
                    except Exception as e:
                        logging.error(f"Failed to send hackathon notifications in guild {guild.id}: {e}")
                else:
...
async def search(interaction: discord.Interaction, keyword: str):
    ...
    try:
        ...
        await interaction.followup.send("❌ An error occurred while searching the database. Please try again later.", ephemeral=True)
        return
    finally:
        db.close()
        ...
        if interaction.channel:
            ...
            if permissions.send_messages and permissions.embed_links:
                try:
                    ...
                    await interaction.followup.send("⚠️ **Permission Error**: I don't have permission to send messages in this channel.", ephemeral=True)
                except Exception as e:
                    logging.error(f"Error sending hackathons to channel: {e}")
                    await interaction.followup.send("⚠️ An error occurred while sending the results.", ephemeral=True)
            else:
                await interaction.followup.send("⚠️ I found hackathons, but I don't have permission to send messages or embeds in this channel.", ephemeral=True)
        else:
            await interaction.followup.send("⚠️ Unable to determine the channel context.", ephemeral=True)
    else:
        logging.info(f"Sending {len(results)} search results to user {interaction.user.id} via DM with pagination")
        try:
...
async def platform(interaction: discord.Interaction, name: str, count: int = 3):
    ...
    try:
        ...
        await interaction.followup.send("❌ An error occurred while fetching hackathons. Please try again later.", ephemeral=True)
        return
    finally:
        db.close()
    ...
    if interaction.guild:
        ...
        logging.info(f"Sending {len(results)} platform results to user {interaction.user.id} via DM with pagination")
        try:
            paginator = HackathonPaginator(results, context_type="dm")
            msg, embed, view_buttons = paginator.create_embed()
            paginator.add_action_buttons(view_buttons)
...
async def upcoming(interaction: discord.Interaction, days: int = 7):
    ...
    try:
        ...
        await interaction.followup.send("❌ An error occurred while fetching upcoming hackathons. Please try again later.", ephemeral=True)
        return
    finally:
        db.close()
    
    if not results:
        await interaction.followup.send(f"❌ No upcoming hackathons found in the next **{days}** days.", ephemeral=True)
        return
    ...
    if interaction.guild:
        ...
        logging.info(f"Sending {len(results)} upcoming results to user {interaction.user.id} via DM with pagination")
        try:
            paginator = HackathonPaginator(results, context_type="dm")
            msg, embed, view_buttons = paginator.create_embed()
            paginator.add_action_buttons(view_buttons)
...
async def about(interaction: discord.Interaction):
    ...
    view = discord.ui.View()
    view.add_item(discord.ui.Button(label="⭐ Star on GitHub", style=discord.ButtonStyle.link, url="https://github.com/Spartan-71/Discord-Hackathon-Bot"))
    view.add_item(discord.ui.Button(label="🐛 Report Bug", style=discord.ButtonStyle.link, url="https://github.com/Spartan-71/Discord-Hackathon-Bot/issues"))
    view.add_item(discord.ui.Button(label="💬 Support Server", style=discord.ButtonStyle.link, url="https://discord.gg/3pM6yJBE"))
